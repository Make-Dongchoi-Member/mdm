<script lang="ts">
  ...
  const makeButtonActivationEvent = () => {
    ...
    if (
      (initialRoomInfo.roomtype === RoomType.LOCK && !isPassword) ||
      (initialRoomInfo.roomtype !== RoomType.LOCK &&
        isPassword &&
        passwordInput !== "") ||
  ...
  const initialInput = () => {
    ...
    initialRoomInfo.roomtype === RoomType.PRIVATE
      ? (isPrivate = true)
      : (isPrivate = false);
    initialRoomInfo.roomtype === RoomType.LOCK
      ? (isPassword = true)
...
<div
  ...
  style={$modalStatesStore.isSettingModal
    ? "display: block;"
    : "display: none;"}
  use:clickOutside
  on:outclick={() => {
...
>
  <div class="modal-title">
    <div>YOUR CHAT ROOM</div>
    <button class="close-button" on:click={closeButtonEvent}>
